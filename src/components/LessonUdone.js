...
export default class LessonUdone extends Component {
    ...
    handleClickQxyy() {
        fetch(CancelLesson+`?DemandId=${this.state.DemandId}`,
            {
                method: "GET",
    ...
    moreClick(num){
        if(! this.state.isMore){
            this.refs.daishangke.style.height = 203 * num +'px';
            this.refs.more.innerHTML = '<< 收起';
            this.setState({
                isMore:true
            })
        }else{
            this.refs.daishangke.style.height = 405 +'px';
            this.refs.more.innerHTML = '展示更多 >>';
            this.setState({
                isMore:false
            })
        }
    ...
    render() {
        ...
        return (
            <div>
                {
                    items.length>=2 ? <ul className="index_panelbox" style={{height:'405px',overflow:'hidden'}} ref="daishangke">
                    {items}
                </ul>
                :
                <ul className="index_panelbox" ref="daishangke">
                    {items}
                </ul>
                }
                
                {
                    dataObj.length>=3? <p ref="more" style={{textAlign:'center',fontSize:'12px',lineHeight:'30px',cursor:'pointer'}} onClick={this.moreClick.bind(this,dataObj.length)}>展示更多 >></p>
                    :
                    ''
                }
                <div className="yy_mengtai" ref="yyMengtai">
                    ...
                    <div className="yys" ref="yyPrompts">

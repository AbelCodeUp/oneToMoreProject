...
export default class MyOrder extends React.Component {
    ...
    componentDidMount() {
        fetch(GetOrderPage+`?pageIndex=${this.state.current}&pageSize=${this.state.pageSize}`,
            {
                headers: {
    ...
    onChange = (page) => {
        fetch(GetOrderPage+`?pageIndex=${page}&pageSize=${this.state.pageSize}`,
            {
                method: "GET",
                headers: {
                    'Authorization': window.localStorage.getItem('Tonken')
                        ...
                        <div className="order_item wait_order">
                            ...
                            <div className="order_item_content">
                                <ul>
                                    ...
                                    <li>
                                        <a target="_blank" href={ConfirmOrderPay+`?&GoodsId=${d.GoodsId}&ClassHours=${d.Hours}&ExpireMou=${d.ExpDay}&GPrice=${d.Prices}&AmountPayable=${d.AmountPayable}&OrderNum=${d.OrderNUM}&GoodsName=${escape(d.GoodsName)}&coupon=${d.AmountPayable - d.Prices}&CouponId=${d.CouponId}&MyNum=${window.localStorage.getItem('Tonken')}`} className="pay_btn">
                                            去支付
                                        </a>
        ...
        return (
            <div className="b_panel">
                <div style={{height:'50px',lineHeight:'50px'}}>
                    <Link to="/Main/Xuzhi" style={{fontSize:'20px',color:'#003df5'}}>《hi翻外教课堂课程协议》</Link>  
                </div>
                <div className="order_header" style={{borderTop:'5px solid #ffab05'}}>
                    <h3 className="fl">
                        我的订单
                       </h3>
                ...
                <iframe src={Auth} style={{display: 'none',height: '1px', width: '1px'}}></iframe>
            </div>
        )
    }
}

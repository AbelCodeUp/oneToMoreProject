...
const downIpad = require('../images/ipddown/ipd-banner.png');
...
export default class IndexMain extends Component {
    ...
    openClick(val) {
        ...
        fetch(GetIntegralState,//是否完善资料与分享朋友圈
            {
                method: "GET",
                headers: {
    ...
    handleReate(flag) {
        ...
        if (flag) {
            fetch(GetOrdinaryStudentInfo,//获取学员信息，判断是否是付费用户
                {
                    method: "GET",
                    headers: {
                        'Authorization': window.localStorage.getItem('Tonken')
            ...
            fetch(GetRecommendedCourses,//推荐预约
                {
                    method: "GET",
                    headers: {
                        Authorization: window.localStorage.getItem('Tonken')
    ...
    closeIpad() {
        ...
        this.refs.ipadvideo.pause();
    }
    palyVideo() {
        this.setState({
            isPlay: true
        })
        this.refs.ipadvideo.play();
    }
    ...
    render() {
        ...
        // ipad
        const ipadShow = this.state.isShowIpad ? { display: 'block' } : { display: 'none' };

        const ipadMask = !this.state.isPlay ? { display: 'block' } : { display: 'none' };
        ...
        return (
            <div>
                ...
                {/* iPad 下载地址*/}
                <div className="ipadDownImg" style={{ cursor: 'pointer' }} onClick={this.openIpad.bind(this)}>
                    <img src={downIpad} />
                </div>
                {/* <!--报名  --> */}
                ...
                < div className="testTing" style={isShowHuoqu} >
                    <div className="wins w_center w_jifen_gz" style={{ padding: '18px 14px' }}>
                        ...
                        <div className="win_body">
                            ...
                            <a style={{ lineHeight: '24px', color: '#ff6600', fontSize: '14px' }} target="_blank" href={Active}>点击去参与》</a>
                        </div>
                    </div>
                </div>
                {/* ipad */}
                <div className="ipad-container" id="ipad-container" style={ipadShow}>
                    <div className="ipad-head">
                        iPad应用下载
                        <div className="ipad-close" id="ipad-close" onClick={this.closeIpad.bind(this)}>
                            &times;
                        </div>
                    </div>
                    <div className="ipad-content">
                        <video controls ref='ipadvideo' src="http://one2more.oss-cn-beijing.aliyuncs.com/guanwang/videos/appGuide.mp4" id="ipadVideo"></video>
                        <div className="ipad-play" ref="playmaskbtn" style={ipadMask}>
                            <img src="../images/downipad/ipad-play1.png" onClick={this.palyVideo.bind(this)} alt="" id="ipad-play-btn" />
                        </div>
                    </div>
                    <div className="ipad-footer">
                        <p>
                            <span style={{ color: '#ff0000' }}>注意事项：</span><br />
                            1、请使用 IPad 在App Store 搜索并下载 <span className="ipad-blue">“hi翻外教课堂”</span>。<br />
                            2、IPad目前只支持上课。<br />
                            3、约课、购买等其他功能，请关注 <span className="ipad-blue">“hi翻外教课堂”</span>公众号或使用电脑、笔记本登录学习中心。<br />
                        </p>
                    </div>
                </div>
                <div className="ipadMask" style={ipadShow} ></div>
            </div>
        )
    }
}